...
public class OMUSERClient {
    ...
    public TransNumberDTLClientResp GetTransNumberDtlByTMasterId(String masterID){
        TransNumberDTLClientResp respData = new TransNumberDTLClientResp();
        try {
            URL url = new URL(String.format(
                    "http://%s%s/trans_number_dtl/by_trans_master_id?trans_master_id=%s",
                    host,
                    context,
                    masterID
                )
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                // Parse JSON response into MetricsResp object using ObjectMapper
                ObjectMapper objectMapper = new ObjectMapper();
                TransNumberDTLData data = objectMapper.readValue(response.toString(), TransNumberDTLData.class);
                respData.setData(data);
            }
        } catch (Exception e) {
            e.printStackTrace();
